...
void OP_Dxyn(){
    ...
    uint8_t x_position = registers[Vx] % SCREEN_LENGTH;
    uint8_t y_position = registers[Vy] % SCREEN_HEIGHT;
    ...
    for (int row=0; row < nibble; row++){
        ...
        for (int column = 0; column < 8; column++){             //sprites are 8 bytes wide
            ...
            if (pixel){
                if (screen[x_position+column+(y_position+row)*SCREEN_LENGTH] == 0xFFFFFFFF){
                    registers[0xF] = 1;
                }
                screen[x_position+column+(y_position+row)*SCREEN_LENGTH] ^= 0xFFFFFFFF;
            }
        } 
    }
    drawflag = 1;
...
void Chip8_init(){
    ...
    function_0[0x0] = &OP_00E0;
    for (uint8_t i=0x1; i < 0xE; i++) function_0[i] = &OP_NULL;
    function_0[0xE] = &OP_00EE;
...
void Window_Manager_Init(char *rom_file){
    ...
    SDL_RenderSetLogicalSize(renderer, SCREEN_LENGTH, SCREEN_HEIGHT);
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
    SDL_RenderClear(renderer);
    texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, SCREEN_LENGTH, SCREEN_HEIGHT);
}
...
void Screen_Draw(){
    uint32_t pixels[SCREEN_LENGTH*SCREEN_HEIGHT];

    if(drawflag){
        memset(pixels, 0, (62*32)*4);
        for(int x=0; x<SCREEN_LENGTH; x++){
            for (int y=0; y<SCREEN_HEIGHT;y++){
                if (screen[x+(y*SCREEN_LENGTH)] == 0xFFFFFFFF){
                    pixels[x+(y*SCREEN_LENGTH)] = UINT32_MAX;
                }
            }
        }
